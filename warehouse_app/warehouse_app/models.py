...
class Order(models.Model):
    ...
    def save(self, sync=False, *args, **kwargs):
        ...
        if sync:
            response = requests.put(f'{self.store_account.uri}/order',
    ...
    def delete(self, sync=False, *args, **kwargs):
        if sync:
            response = requests.delete(f'{self.store_account.uri}/order', params={'sync': False,
                                                                                  'order_number': self.order_number})
    ...
    def __str__(self):
        return f'{self.order_number} - {self.store_account}'
