...
from .serializers import OrderSerializer, BaseParamsSerializer, \
    DestroyParamsSerializer


# TODO: move serializers.is_valid to methods valid_params and valid_data
...
@api_view(['GET'])
def warehouse_exists(request):
    # TODO: serialize query params
    warehouse = WarehouseAccounts.warehouses.filter(name=request.query_params['name']).first()
    return Response({'is_exists': bool(warehouse)},
                    status=status.HTTP_404_NOT_FOUND if not warehouse else status.HTTP_200_OK)